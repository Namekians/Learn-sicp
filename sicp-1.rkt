...
(define (repeated-iter f n)
  (define (iter res count)
     (if (= count 1) res (iter (compose res f) (- count 1)))
    )
  (iter f n)
  )
(define (repeated f n)
...
(define (iterative-improve good-enough? improve)
  (lambda (x)(if (good-enough? x (improve x)) x ((iterative-improve good-enough? improve) (improve x))))
  )
